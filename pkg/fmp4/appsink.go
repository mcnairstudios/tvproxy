...
func SetupVideoSinkSC(pipeline *gst.Pipeline, sc *SegmentController) error {
	elem, err := pipeline.GetElementByName("videosink")
	if err != nil {
		return err
	}
	...
	sink.SetCallbacks(&app.SinkCallbacks{
		NewSampleFunc: func(s *app.Sink) gst.FlowReturn {
			...
			if sc.videoCodec == "av1" && len(data) <= 10 {
				return gst.FlowOK
			}
			...
			sc.PushVideoFrame(data, ptsNs, bufDurNs, bufKeyframe)
			return gst.FlowOK
		},
	})
	return nil
...
func SetupAudioSinkSC(pipeline *gst.Pipeline, sc *SegmentController) error {
	...
	sink.SetCallbacks(&app.SinkCallbacks{
		NewSampleFunc: func(s *app.Sink) gst.FlowReturn {
			...
			sc.PushAudioFrame(data, ptsNs, bufDurNs)
			return gst.FlowOK
		},
	})
	return nil
