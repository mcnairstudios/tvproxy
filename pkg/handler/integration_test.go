...
func setupFullEnv(t *testing.T) *fullTestEnv {
	...
	m3uService := service.NewM3UService(m3uAccountStore, streamStore, channelStore, logoService, nil, cfg, dir, nil, log)
	...
	proxyService := service.NewProxyService(channelStore, streamStore, profileStore, clientService, activityService, settingsService, nil, cfg, nil, log)
